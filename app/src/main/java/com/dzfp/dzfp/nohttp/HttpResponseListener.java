...
public class HttpResponseListener<T> implements OnResponseListener<T> {
  ...
  /*      if (isLoading) {// 需要显示dialog
    ...
    public void onSucceed(int what, Response<T> response) {
        if (rtnEcuInfoBean(response.get().toString()).equals("请登陆后再使用") && CustomApplication.isLogin) {
            DialogHb.showdialog(context, "该账号已在其他地方登录，或登录超时");
    ...
    private String rtnEcuInfoBean(String resutl) {
        String resultType = "";
        String msg = "";
        if (resutl != null) {
            JSONObject jsonObject = null;
            try {
                jsonObject = new JSONObject(resutl);
