    ...
    public void getCode() {
        ...
        CallServer.getRequestInstance().add(0, mContext, mRequest, new HttpListener<JSONObject>() {
            ...
            public void onSucceed(int what, Response<JSONObject> response) {
                ResultRoot resultRoot  = new Gson().fromJson(response.get().toString(),ResultRoot.class);
                if(resultRoot.getResultType().equals("SUCCESS")){
                    mView.getCodeSuccess();
                }
                Log.i(TAG, "getCode: " + response.get().toString());
